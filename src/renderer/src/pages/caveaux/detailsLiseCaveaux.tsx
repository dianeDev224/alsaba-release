...
import {
  TableHistoriquesCaveaux,
} from '@renderer/components/tables/tableCaveaux';
import { LoaderContext } from '@renderer/hooks/loader';
import { supabase } from '@renderer/repository/connection';

// import { TableHistoriquesCaveaux } from '@renderer/componente/TableHistoriquesCaveaux';

export default function DetailListeCaveaux() {
      
      const [expendDetails , setExpendDetails] = useState(true)
      const [activateEdition , setActivateEdition] = useState(false)
      const [formValid , setFormValid] = useState(false)
      const [hisotiqueData , setHistoriqueData] = useState(null)
          ...
          <div className='flex h-[470px] flex-col px-[20px] space-y-4 overflow-y-scroll  scrollbar-thin scrollbar-track-zinc-800 scrollbar-thumb-orange-600 scrollbar-corner-gray-300'>
              ...
              <div className='flex flex-row w-full justify-between items-center'>
                  ...
                  <div className='flex flex-row w-[200px] space-x-2 items-center h-[60px]'>
                      ...
                      <h3 className='font-light uppercase text-[#5f6163]'>Détails du caveau</h3>
                  </div>
                  {/* right side */}
              ...
              <div className='flex flex-2 basis-[65%] h-[360px] flex-col items-center py-2 space-y-3 bg-[#fff]  border-[3px] border-[#cecbcb] rounded-md'>
                  {/* titre */}
                  <p className='flex flex-row text-[#e98033] cursor-pointer'>Info Détails <BsFillPencilFill className='mx-2' onClick={()=>setActivateEdition(true)} /> </p>
                  {/* nom pays */}
                  <div>
                      <InputText
              ...
              <h3 className='font-light uppercase text-[#5f6163] w-full text-center'>Les Historiques  de ce caveaux</h3>
              {/* les historiques de ce  caveaux */}
              <TableHistoriquesCaveaux data={hisotiqueData} />
          </div>
      )
  }
